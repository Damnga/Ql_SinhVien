...
namespace Ql_SinhVien
{
    public partial class FormLopHoc : Form
    {
        ...
        {
            conn = new SqlConnection(nguon);
            sql = @"select KhoaHoc.MaKhoa,TenKhoa,NganhHoc.MaNganh,TenNganh,MaLop,TenLop from KhoaHoc, NganhHoc,LopHoc
        ...
        private bool IsDataDuplicate(string newData, string tenngang, string tenlop)
        {
            try
            {
                conn = new SqlConnection(nguon);
                sql = "SELECT COUNT(*) FROM KhoaHoc,NganhHoc,LopHoc WHERE KhoaHoc.MaKhoa=NganhHoc.MaKhoa and NganhHoc.MaNganh=LopHoc.MaNganh and TenKhoa=@ten and TenNganh=@tennganh and TenLop = @tenlop";
                cmd = new SqlCommand(sql, conn);
        ...
        private void comboBox2_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (comboBox2.SelectedItem != null)
            {
                ...
                cmd = new SqlCommand(sql, conn);
                cmd.Parameters.AddWithValue("@TenKhoa", selectIdCombobox(comboBox2, "MaKhoa"));
                ...
                conn = new SqlConnection(nguon);
                conn.Open();
                sql = @"select KhoaHoc.MaKhoa ,TenKhoa,NganhHoc.MaNganh,TenNganh,MaLop,TenLop from KhoaHoc,NganhHoc,LopHoc where KhoaHoc.MaKhoa=NganhHoc.MaKhoa and NganhHoc.MaNganh=LopHoc.MaNganh and KhoaHoc.MaKhoa=@MaKhoa";
                SqlDataAdapter adapter = new SqlDataAdapter(sql, conn);
                adapter.SelectCommand.Parameters.AddWithValue("@MaKhoa", selectIdCombobox(comboBox2, "MaKhoa"));
                DataTable dataTable = new DataTable();
                adapter.Fill(dataTable);
                dataGridView1.DataSource = dataTable;
                conn.Close();
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                comboBox2.Text = tenKhoa;
                comboBox1.Text = tenNganh;
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                if (IsDataDuplicate(comboBox2.Text, comboBox1.Text, textBox1.Text))
                {
                    MessageBox.Show("Dữ liệu đã tồn tại!");
                    return;
                }
                conn = new SqlConnection(nguon);
                sql = @"insert into LopHoc (TenLop,MaNganh) values(@TenLop,@MaNganh)";
                cmd = new SqlCommand(sql, conn);
                cmd.Parameters.AddWithValue("@TenLop", textBox1.Text);
                cmd.Parameters.AddWithValue("@MaNganh", selectIdCombobox(comboBox1, "MaNganh"));
                conn.Open();
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                conn = new SqlConnection(nguon);
                sql = @"update LopHoc set TenLop=@TenLop,MaNganh=@MaNganh where MaLop=@MaLop";
                cmd = new SqlCommand(sql, conn);
                cmd.Parameters.AddWithValue("@TenLop", textBox1.Text);
            ...
            {
                MessageBox.Show("Bạn cần phải xóa sinh vien thuộc lớp này trước!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
